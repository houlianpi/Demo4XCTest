...
echo "========================="
echo "xcodebuild -sdk iphonesimulator -target ${UNIT_TEST_TARGET} clean build CONFIGURATION_BUILD_DIR=\"${OUTPUT_DIR}\""
echo "========================="
xcodebuild -sdk iphonesimulator -target "${UNIT_TEST_TARGET}" -arch i386 -configuration Debug clean build CONFIGURATION_BUILD_DIR="${OUTPUT_DIR}"
if [[ $? != 0 ]]; then
...
echo "==========================="
echo "xcodebuild -sdk iphonesimulator -target ${MAIN_APP_TARGET} clean build CONFIGURATION_BUILD_DIR=\"${OUTPUT_DIR}\" XCODE_TEST_LDFLAGS=\"${XCODE_TEST_LDFLAGS}\""
echo "==========================="
xcodebuild -sdk iphonesimulator -target "${MAIN_APP_TARGET}" -arch i386 -configuration Debug build CONFIGURATION_BUILD_DIR="${OUTPUT_DIR}" XCODE_TEST_LDFLAGS="${XCODE_TEST_LDFLAGS}"
if [[ $? != 0 ]]; then
